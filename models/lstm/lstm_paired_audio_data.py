...
class PairedLSTMDataset(torch.utils.data.Dataset):
    """
    Dataset that serves paired audio: (clean, processed) for LSTM model.
    ...
    def __init__(self,
        ...
        self.target_length = target_length # Same as item_length for LSTM
        self.train = train
        ...
        # build index over processed arrays
        ...
        # For LSTM, we take L samples of clean and L samples of processed.
    ...
    def _create_paired_npz(self, clean_dir: str, processed_dir: str, out_file: str):
        print(f"Creating paired LSTM dataset from\n  clean_dir: {clean_dir}\n  processed_dir: {processed_dir}")
        ...
        print(f"Saved paired LSTM npz: {out_file} with {len(common)} pairs")
    ...
    def __getitem__(self, idx: int):
        ...
        # LSTM Input: [L, 1] (Sequence Length, Features)
        lstm_input = clean_example.unsqueeze(-1) 
        
        # LSTM Target: [L, 1] (Sequence Length, Features) - Flattened to [L] for loss
        lstm_target = proc_example.unsqueeze(-1)
        
        return lstm_input, lstm_target
